...
describe('/api/people routes', () => {
    ...
    it('should retrieve all people if no params are given', () => {
      ...
      return Promise.all([Person.create(person1), Person.create(person2)]).then(
        () => {
          ...
          return (
            request(app) // have to return this promise as well
              .get('/api/people')
  ...
  describe('POST to /api/people', () => {
    it('should create a new person and return that persons information if all the required information is given', async () => {
      ...
      const allPeopleBefore = await Person.findAll();
      const postResponse = await request(app).post(
        '/api/people/?name=nick&is_attending=true'
      );
      expect(postResponse.body.name).toEqual('nick');
      const allPeopleAfter = await Person.findAll();
      expect(allPeopleAfter.length).toEqual(allPeopleBefore.length + 1);
    });
    it('should return status code 400 if missing required information', async () => {
      const badPostResponse = await request(app).post(
        '/api/people/?is_attending=true'
      );
      expect(badPostResponse.status).toEqual(400);
    });
  });

  describe('PUT to /api/people/:id', () => {
    it('should update a persons information', async () => {
      // insert a person so you have someone to update
      const testPerson = await request(app).post(
        '/api/people/?name=nick&is_attending=true'
      );
      const putRequestResponse = await request(app).put(
        '/api/people/?id=1&is_attending=false&name=new+nick'
      );

      if (putRequestResponse.body.error) return;
      expect(putRequestResponse.body[1][0].id).toEqual(1);
      expect(putRequestResponse.body[1][0].isAttending).toEqual(false);
      expect(putRequestResponse.body[1][0].name.toLowerCase()).toEqual(
        'new nick'
      );
    });
    it('should return a 400 if given an invalid id', async () => {
      const badPutRequest = await request(app).put(
        '/api/people/?id=abc&is_attending=false&name=new+nick'
      );
      expect(badPutRequest.status).toEqual(400);
    });
  });
  ...
  describe('DELETE to /api/people/:id', () => {
    it('should remove a person from the database', async () => {
      ...
      const deletedRows = await request(app).delete('/api/people/?id=1');
      expect(deletedRows.text).toEqual('1');
      //expecting a number causes jest to throw this error:  Async callback was not invoked within the 5000ms timeout specified by jest.setTimeout.Timeout
    });
    it('should return a 400 if given an invalid id', async () => {
      const badDeleteRequest = await request(app).delete('/api/people/?id=abc');
      expect(badDeleteRequest.status).toEqual(400);
    });
  });
});
